"use client";

import { aboutContent } from "@/content/about";
import { useState } from "react";

export default function QuemSouEuPage() {
  const [language, setLanguage] = useState<"pt" | "en">("pt");
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [menuOpen, setMenuOpen] = useState(false);
  const content = aboutContent[language];

  const nextImage = () => {
    setCurrentImageIndex((prev) =>
      prev === aboutContent.images.length - 1 ? 0 : prev + 1,
    );
  };

  const prevImage = () => {
    setCurrentImageIndex((prev) =>
      prev === 0 ? aboutContent.images.length - 1 : prev - 1,
    );
  };

  const goToImage = (index: number) => {
    setCurrentImageIndex(index);
  };

  return (
    <main className="min-h-screen bg-white text-zinc-900">
      {/* HEADER */}
      <header className="flex items-center justify-between border-b px-4 py-3">
        {/* LEFT SIDE: Hamburger Menu + Logo + Name */}
        <div className="flex items-center gap-3">
          {/* Hamburger Menu */}
          <button
            onClick={() => setMenuOpen(!menuOpen)}
            className="flex flex-col gap-1.5 p-2 hover:bg-gray-100 rounded-md transition-colors"
            aria-label="Menu"
          >
            <span
              className={`block h-0.5 w-6 bg-zinc-900 transition-all ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
            ></span>
            <span
              className={`block h-0.5 w-6 bg-zinc-900 transition-all ${menuOpen ? "opacity-0" : ""}`}
            ></span>
            <span
              className={`block h-0.5 w-6 bg-zinc-900 transition-all ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            ></span>
          </button>

          {/* Logo */}
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 p-2">
            <svg viewBox="0 0 100 100" className="h-full w-full">
              <circle
                cx="50"
                cy="25"
                r="18"
                fill="none"
                stroke="url(#grad1)"
                strokeWidth="2"
              />
              <circle
                cx="30"
                cy="55"
                r="18"
                fill="none"
                stroke="url(#grad2)"
                strokeWidth="2"
              />
              <circle
                cx="70"
                cy="55"
                r="18"
                fill="none"
                stroke="url(#grad3)"
                strokeWidth="2"
              />
              <defs>
                <linearGradient id="grad1" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#9333ea" />
                  <stop offset="100%" stopColor="#ec4899" />
                </linearGradient>
                <linearGradient id="grad2" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#ec4899" />
                  <stop offset="100%" stopColor="#f97316" />
                </linearGradient>
                <linearGradient id="grad3" x1="0%" y1="0%" x2="100%" y2="0%">
                  <stop offset="0%" stopColor="#f97316" />
                  <stop offset="100%" stopColor="#fb923c" />
                </linearGradient>
              </defs>
            </svg>
          </div>

          {/* Name */}
          <div className="font-semibold">Taróloga Calil</div>
        </div>

        {/* LANGUAGE SWITCHER */}
        <div className="flex gap-2">
          <button
            onClick={() => setLanguage("pt")}
            className={`px-3 py-1 text-sm rounded-md transition-colors ${
              language === "pt"
                ? "bg-violet-600 text-white"
                : "text-zinc-500 hover:bg-zinc-100"
            }`}
          >
            PT
          </button>
          <button
            onClick={() => setLanguage("en")}
            className={`px-3 py-1 text-sm rounded-md transition-colors ${
              language === "en"
                ? "bg-violet-600 text-white"
                : "text-zinc-500 hover:bg-zinc-100"
            }`}
          >
            EN
          </button>
        </div>
      </header>

      <section className="mx-auto max-w-2xl px-4 py-8">
        {/* TITLE WITH ICON */}
        <div className="mb-6 flex items-center justify-center gap-3">
          <span className="text-4xl">🔮</span>
          <h1 className="text-center text-2xl font-semibold text-violet-700">
            {content.title}
          </h1>
        </div>

        {/* VIDEO */}
        <div className="mb-8 aspect-video overflow-hidden rounded-xl bg-black shadow-lg">
          <iframe
            src={aboutContent.videoUrl}
            title="Quem sou eu - Taróloga Calil"
            className="h-full w-full"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
            allowFullScreen
          />
        </div>

        {/* IMAGE CAROUSEL */}
        <div className="relative mx-auto mb-4 max-w-md">
          {/* Main Image Container */}
          <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-gray-100 shadow-md">
            <img
              src={aboutContent.images[currentImageIndex]}
              alt={`Taróloga Calil - Image ${currentImageIndex + 1}`}
              className="h-full w-full object-cover"
            />

            {/* Previous Button */}
            <button
              onClick={prevImage}
              className="absolute left-4 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-violet-700 shadow-lg transition-all hover:bg-white hover:scale-110"
              aria-label="Previous image"
            >
              <svg
                className="h-6 w-6"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M15 19l-7-7 7-7"
                />
              </svg>
            </button>

            {/* Next Button */}
            <button
              onClick={nextImage}
              className="absolute right-4 top-1/2 -translate-y-1/2 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-violet-700 shadow-lg transition-all hover:bg-white hover:scale-110"
              aria-label="Next image"
            >
              <svg
                className="h-6 w-6"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M9 5l7 7-7 7"
                />
              </svg>
            </button>

            {/* Image Counter */}
            <div className="absolute bottom-4 right-4 rounded-full bg-black/50 px-3 py-1 text-sm text-white">
              {currentImageIndex + 1} / {aboutContent.images.length}
            </div>
          </div>
        </div>

        {/* Thumbnail Navigation */}
        <div className="mb-8 flex justify-center gap-3 overflow-x-auto pb-4">
          {aboutContent.images.map((img, index) => (
            <button
              key={img}
              onClick={() => goToImage(index)}
              className={`h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg transition-all ${
                index === currentImageIndex
                  ? "ring-4 ring-violet-500 shadow-lg scale-105"
                  : "opacity-60 hover:opacity-100 hover:scale-105"
              }`}
            >
              <img
                src={img}
                alt={`Thumbnail ${index + 1}`}
                className="h-full w-full object-cover"
              />
            </button>
          ))}
        </div>

        {/* Pagination Dots */}
        <div className="mb-10 flex justify-center gap-2">
          {aboutContent.images.map((_, index) => (
            <button
              key={index}
              onClick={() => goToImage(index)}
              className={`h-2 w-2 rounded-full transition-all ${
                index === currentImageIndex
                  ? "bg-violet-500 w-8"
                  : "bg-violet-200 hover:bg-violet-300"
              }`}
              aria-label={`Go to image ${index + 1}`}
            />
          ))}
        </div>

        {/* FORMATION SECTION */}
        {content.formation && (
          <div className="mb-8 rounded-lg bg-violet-50 p-6 font-[family-name:var(--font-cinzel)]">
            <h2 className="mb-4 text-xl font-semibold text-violet-700">
              {content.formationTitle}
            </h2>
            <p className="whitespace-pre-line text-sm leading-relaxed text-zinc-700">
              {content.formation}
            </p>
          </div>
        )}

        {/* DESCRIPTION */}
        <div className="mb-10 font-[family-name:var(--font-cinzel)]">
          <p className="whitespace-pre-line text-sm leading-relaxed text-zinc-700">
            {content.description}
          </p>
        </div>

        {/* CONTACT */}
        <div className="text-center">
          <p className="mb-6 text-sm italic text-zinc-600">
            {content.contactText}
          </p>

          <div className="flex justify-center gap-6">
            <a
              href={aboutContent.contacts.whatsapp}
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-12 w-12 items-center justify-center rounded-full bg-green-500 text-white transition-transform hover:scale-110"
              aria-label="WhatsApp"
            >
              <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
            </a>

            <a
              href={aboutContent.contacts.instagram}
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 text-white transition-transform hover:scale-110"
              aria-label="Instagram"
            >
              <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            </a>

            <a
              href={aboutContent.contacts.youtube}
              target="_blank"
              rel="noopener noreferrer"
              className="flex h-12 w-12 items-center justify-center rounded-full bg-red-600 text-white transition-transform hover:scale-110"
              aria-label="YouTube"
            >
              <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
              </svg>
            </a>
          </div>
        </div>
      </section>
    </main>
  );
}
